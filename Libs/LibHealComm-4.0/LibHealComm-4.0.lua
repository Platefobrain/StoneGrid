				...
				if( pending.bitType and bit.band(pending.bitType, bitFlag) > 0 ) then
					for i=1, #(pending), 5 do
						...
						elseif( ( pending.bitType == CHANNEL_HEALS or pending.bitType == HOT_HEALS ) and ( not pending.hasVariableTicks or pending.hasVariableTicks and amount[1] ) ) then
							...
							-- Anchor on ticksLeft/endTime rather than a raw modulo -- see the
							-- matching fix and explanation in GetHotTickAmount above. A plain
							-- `secondsLeft % tickInterval` reports 0 ("ticking right now")
							-- whenever secondsLeft happens to land on an exact multiple of
							-- tickInterval, which is exactly true at the moment of casting.
							local untilNextTick
							if( ticksLeft and ticksLeft > 0 ) then
								untilNextTick = (endTime - currentTime) - (ticksLeft - 1) * pending.tickInterval
							else
								untilNextTick = secondsLeft % pending.tickInterval
							end
							if( untilNextTick < 0 ) then untilNextTick = 0 end
							local nextTick = currentTime + untilNextTick
...
-- Returns a table of active HoTs on the given targetGUID with per-spell tick info.
-- Each entry: { spellID, casterGUID, nextTickIn, tickAmount, tickInterval, ticksLeft }
-- nextTickIn  = seconds until the next tick of this specific HoT (0 if ticking right now)
-- tickAmount  = heal value of one tick
-- tickInterval = seconds between ticks for this spell
-- ticksLeft   = number of ticks remaining (including the next one)
-- Usage:
--   local hots = HealComm:GetHotTickAmount(targetGUID)
		...
		for _, pending in pairs(pendingHeals[casterGUID]) do
			if( pending.bitType == HOT_HEALS ) then
				for i = 1, #(pending), 5 do
					...
					local ticksLeft = pending[i + 4]
					endTime = endTime > 0 and endTime or pending.endTime
					if( endTime <= currentTime and pending.endTime and pending.endTime > currentTime ) then
						endTime = pending.endTime
						if( not ticksLeft or ticksLeft <= 0 ) then
							ticksLeft = math.ceil((endTime - currentTime) / pending.tickInterval)
						end
					end

					if( guid == targetGUID and pending.tickInterval and pending.tickInterval > 0 and ( endTime > currentTime or (ticksLeft and ticksLeft > 0) ) ) then
						local secondsLeft = endTime - currentTime

						-- The remaining ticksLeft ticks are evenly spaced and land on endTime,
						-- so the next one is due (ticksLeft - 1) intervals before endTime.
						-- NOTE: this used to be `secondsLeft % tickInterval`, which is wrong
						-- whenever secondsLeft is an exact multiple of tickInterval -- which is
						-- exactly true right at the moment a HoT is cast (endTime = cast time +
						-- totalTicks * tickInterval) and right after every real tick resets
						-- endTime. That made GetHotTickAmount report nextTickIn = 0 ("tick is
						-- landing right now") at the instant of casting, when in reality the
						-- first real tick is still a full tickInterval away. Spells that also
						-- grant an instant heal on cast (e.g. Rejuvenation + Conqueror's/
						-- Valorous/Sanctified Nightsong Regalia 4pc) then visibly land on the
						-- target's health at the same moment the UI was (wrongly) showing that
						-- same tick as still "incoming", making it look like the heal was being
						-- counted twice.
						local nextTickIn
						if( ticksLeft and ticksLeft > 0 ) then
							nextTickIn = secondsLeft - (ticksLeft - 1) * pending.tickInterval
						else
							nextTickIn = secondsLeft
						end
						if( nextTickIn < 0.01 ) then nextTickIn = 0 end

						local tickAmount
						local baseAmount
						if( type(amount) == "table" ) then
							baseAmount = tonumber(amount[1]) or 0
						else
							baseAmount = tonumber(amount) or 0
						end
						tickAmount = baseAmount * (stack or 1)
						...
						result[#result + 1] = {
							...
							tickInterval = pending.tickInterval,
							ticksLeft    = ticksLeft,
						}
					end
				end
...
function HealComm:UNIT_AURA(unit)
	...
	if not guid or not guidToUnit[guid] then return end
...
local function parseHotHeal(casterGUID, wasUpdated, spellID, tickAmount, totalTicks, tickInterval, ...)
	...
	-- findAura requires UnitIsVisible() on the target so it can scan the aura with
	-- UnitBuff(). In a raid or battleground, a perfectly valid heal target is very
	-- often out of "visible" range (a stricter check than actual heal range), which
	-- silently dropped every HoT prediction for them -- this is why direct heals kept
	-- working while HoTs mostly stopped showing once the group got bigger/spread out.
	-- Fall back to the numbers already computed by CalculateHotHealing/GetHealTargets
	-- instead of discarding the HoT outright; we just lose exact existing-stack info.
	if( not stack or not duration or not endTime ) then
		if( not tickInterval or tickInterval <= 0 ) then return end
		duration = totalTicks and (totalTicks * tickInterval) or tickInterval
		endTime  = GetTime() + duration
		stack    = 1
	elseif( not wasUpdated and tickInterval and tickInterval > 0 and totalTicks ) then
		-- A new/refreshed HoT can briefly read a nearly-expired aura from UnitBuff
		-- (stale timing from the previous HoT or combat-log ordering).  That leaves
		-- ticksLeft at 1 with endTime a fraction of a second away, so incoming heal
		-- flashes and vanishes when the first (often instant) tick lands.
		local expectedDuration = totalTicks * tickInterval
		local remaining = endTime - GetTime()
		if( remaining < expectedDuration * 0.75 ) then
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(timestamp, eventType, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
	...
	if( eventType == "SPELL_PERIODIC_HEAL" ) then
		...
		if( pending and pending[destGUID] and pending.bitType and bit.band(pending.bitType, OVERTIME_HEALS) > 0 ) then
			...
			if( amount ) then
			ticksLeft = (ticksLeft or 0) - 1
			if( ticksLeft > 0 ) then
				endTime = GetTime() + pending.tickInterval * ticksLeft
			elseif( pending.endTime and pending.endTime > GetTime() + 0.05 ) then
				endTime = pending.endTime
				ticksLeft = math.ceil((endTime - GetTime()) / pending.tickInterval)
			else
				endTime = GetTime()
				ticksLeft = 0
			end
...
local function ResolveCastTargetGUID(castTarget)
	if( not castTarget ) then return end

	local guid = UnitGUID(castTarget)
	if( guid ) then return guid end

	if( UnitName("player") == castTarget ) then return UnitGUID("player") end

	for i = 1, 4 do
		local unit = "party" .. i
		if( UnitExists(unit) and UnitName(unit) == castTarget ) then
	...
	if( UnitName("target") == castTarget ) then return UnitGUID("target") end
	if( UnitName("focus") == castTarget ) then return UnitGUID("focus") end
	if( UnitName("mouseover") == castTarget ) then return UnitGUID("mouseover") end
	if( UnitName("targettarget") == castTarget ) then return UnitGUID("target") end
	if( UnitName("focustarget") == castTarget ) then return UnitGUID("focustarget") end
	if( lastFriendlyName == castTarget ) then return lastFriendlyGUID end
end
...
function HealComm:UNIT_SPELLCAST_SENT(unit, spellName, spellRank, castOn)
	...
	if( hadTargetingCursor ) then
		...
		guidPriorities[lastSentID] = nil
		setCastData(0, nil, ResolveCastTargetGUID(castTarget))
	end
end
