...
StoneGrid_RaidDebuffs = {}

local RDDB = StoneGrid_RaidDebuffsDB or {}
...
local function CollectZoneSpells(cfg, zone)
    ...
    local custom = cfg.RaidDebuffCustom
...
function StoneGrid_RaidDebuffs:UpdateZoneSpells()
...
function StoneGrid_RaidDebuffs:FindRaidDebuff(unit)
...
function StoneGrid_RaidDebuffs:IsRaidDebuff(name)
    if not name or not next(spells_order) then return false end
    return spells_order[name:lower()] ~= nil
end

function StoneGrid_RaidDebuffs:InitDefaults(cfg)
    if cfg.RaidDebuffCustom == nil then cfg.RaidDebuffCustom = "" end
end
